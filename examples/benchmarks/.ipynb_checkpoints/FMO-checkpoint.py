...
s   = np.identity(H.shape[0], dtype=complex)      # System-bath coupling matrix
lam = np.array([35] * H.shape[0])                 # Reorganization energies in [cm^-1]
mu  = np.array([0] * H.shape[0])                  # Counter terms in [cm^-1]
wc  = (50 * fs)**(-1)                             # Bath cut-off frequency in [1/ps]
...
#==============================================================================
#--- S-QuAPI paramters
Dkmax = 4               # The maximum number of memory steps
Nmax  = 40              # The maximu times step
Dt    = 850 * fs / 40   # Time slice of path integral
theta = 5.e-6

#--- MPI parameters (only for mode = 'mpi')
nprocs = 2
...
# ----- Generate system.dat --------------------
if system == True:
    sq.getsystem(H, s, lam, mu, T, Dt, Dkmax, g)
