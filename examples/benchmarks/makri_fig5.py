...
Description: Reproduction of Figrue 5 of N. Makri and D. E. Makarov,
             Journal of Chemical Physics 102 4600-4610 (1995),
             page 4609.
Notes:
    - Run this by $ python3 makri_fig5.py
    - Will take several minutes to complete with squapi_omp on
      a two-core system.
"""
...
eps   = Omega                # $\epsilon$ [cm^-1 / hbar]
Dt    = 0.15 / Omega         # $\Delta t$, the time slice [1/ps]
wc    = 7.5 * Omega          # cut-off frequency [1/ps] 
beta  = 5.0 / (hbar * Omega) # inverse temp
xi    = 0.1                  # Kondo parameter
tmax  = 15 / Omega           # the maximum time in units of 1/Omega
Dkmax_list = [1, 3, 5, 7, 9] # Dkmax values used 
#********************************************************************
...
for i in range(len(rhos_list)):
    ...
    plt.plot(x, y_z, legend, label = 'Dkmax=' + str(Dkmax))
plt.ylim(-1, 1)
